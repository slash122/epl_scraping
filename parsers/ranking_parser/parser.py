...
class RankingParser(BaseParser):
    ...
    def parse_location_rankings(self):
        ...
        for province in self.location_data:
            ...
            province_rankings['cities'] = RankingParser.parse_province_rankings(self, province)
    ...
    def parse_province_rankings(self, province):
        ...
        for city in province['cities']:
            self.logger.info(f"Parsing advertisements for {province['name']} - {city['name']}")
    ...
    def parse_city_rankings(self, province, city):
        ...
        while page_number < RankingParser.PAGE_LIMIT:
            ...
            advertisements.extend(RankingParser.create_advertisements(html, page_number))
            if not RankingParser.check_next_page(html): break
            page_number += 1
        return advertisements

    def request_page(self, province, city, page_number):
        page_slug = f"page{page_number}.html" if page_number > 1 else ""
        response = requests.get(RankingParser.RANKING_URL.format(
            city_slug=city['slug'],
            page=page_slug,
    ...
    def create_advertisements(html, page_number):
        advertisements = []
        for index, node in enumerate(html.xpath(RankingParser.XPATHS['advertisements']), start=1):
            advertisements.append(RankingParser.create_advertisement_dict(node, index, page_number))
        return advertisements
