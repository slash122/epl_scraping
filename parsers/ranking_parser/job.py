...
LOCATION_RESULTS_FOLDER = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../results/location_results")
)
...
def run(location_file=None):
    ...
    RankingParser(
        location_data=location_data, cookies=get_cookies(), logger=setup_logger()
    ).run()
...
def get_location_data(location_file=None):
    if not location_file:
        location_file = os.path.join(
            LOCATION_RESULTS_FOLDER, sorted(os.listdir(LOCATION_RESULTS_FOLDER))[-1]
        )
...
def cli_args():
    ...
    cli_parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to the location JSON file",
    )
